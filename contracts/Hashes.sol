...
contract Hashes is IHashes, ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function deactivateTokens(
        ...
        require(
            ...
            "Hashes: The token owner must approve the deactivation."
        );
     ...
     */
    function generate(string memory _phrase) external payable nonReentrant {
     ...
     */
    function getHash(uint256 _tokenId) external view override returns (bytes32) {
     ...
     */
    function getPriorVotes(address _account, uint256 _blockNumber) external view override returns (uint256) {
    ...
    function _beforeTokenTransfer(
        ...
        if (firstTokenId < governanceCap && !deactivated[firstTokenId]) {
    ...
    function deactivateToggle(uint256 tokenId) external onlyOwner {
        if (deactivated[tokenId]) {
            deactivated[tokenId] = false;
        }
        else {
            deactivated[tokenId] = true;
